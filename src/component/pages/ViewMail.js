...
const ViewMail = (props) => {
  ...
  const deleteMailHandler = async () => {
    ...
    if (props.type === "recevied") {
      url = `https://mailbox-4c22f-default-rtdb.firebaseio.com/rec${props.email}/${props.mail.id}.json`;
      dispatch(mailActions.deleteReceivedMail(props.mail.id));
    } else {
      url = `https://mailbox-4c22f-default-rtdb.firebaseio.com/sent${props.email}/${props.mail.id}.json`;
